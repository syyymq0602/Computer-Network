// =====================================================================
// Copyright 2013-2016 Fluffy Underware
// All rights reserved
// 
// http://www.fluffyunderware.com
// =====================================================================

using UnityEngine;
using System.Collections;
using FluffyUnderware.Curvy.Controllers;
using System.IO;
using System.Collections.Generic;

namespace FluffyUnderware.Curvy.Examples
{
    public class SplineControllerInputRail : MonoBehaviour
    {
        public float acceleration = 0.1f;
        public float limit = 30f;
        public SplineController splineController;
        public static float acc;

        //private List<float> list = new List<float>();

        private IEnumerator Start ()
        {
            while (!splineController.IsInitialized)
                yield return 0;
            /*ReadText();
            Speed_Change();
            list = null;*/
        }

        private void Update()
        {
            float velVert = Mathf.Clamp (Input.GetAxis("Vertical"), -1f, 1f);
            splineController.Speed = Mathf.Clamp (splineController.Speed + velVert * acceleration * Time.deltaTime, 0.001f, limit);
            acc = velVert * acceleration * Time.deltaTime;
        }

        /*private void Speed_Change()
        {
            for (int i = 1; i < list.Count; i += 7)
            {
                acc = list[i];
                splineController.Speed = splineController.Speed + acc * 0.0505f;
                StartCoroutine(delay(1f));
                //if (i > 410) i = 1;
            }
        }*/

        /*private void ReadText()
        {
            StreamReader rd = File.OpenText("C:\\Users\\lonovo\\Desktop\\DST\\DataF\\Output\\TrainDefine_CoupledTrack_Acc.txt");
            string s = rd.ReadToEnd();
            string[] tmp = s.Replace("\r\n", "").Replace("\t", "").Replace("\n", "").Trim().Split();
            for (int i = 0; i < tmp.Length;i++ )
            {
                if (i % 8 == 7) continue;
                list.Add(float.Parse(tmp[i]));
            }
        }*/

        /*IEnumerator delay(float time)
        {
            yield return new WaitForSeconds(time);
        }*/
    }
}
